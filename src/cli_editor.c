...
static void cli_escape_input(cli_context_t *ctx, char c);

static void cli_cursor_move_right(cli_context_t *ctx);
static void cli_cursor_move_left(cli_context_t *ctx);
    ...
    else
    {
        ...
        else
        {
            ...
            if ((SPC <= c) && (c <= 0x7e))
            {
                ...
                /* プロンプトの文字列サイズを取得 */
                size = (uint8_t)strlen(ctx->prompt);
                ...
                cli_printf(ctx, "\r");                                      /* 復帰(左寄せ) */
                cli_printf(ctx, "\e[%dC", ctx->cmd_line.cursor.y + size);   /* 現在のカーソル位置に移動 */
            }
            else if (BS == c)
            {
                /* バックスペース */
...
static void cli_escape_input(cli_context_t *ctx, char c)
{
    ...
    if (('[' == ctx->cmd_line.escape.buf[0]) && (2 <= ctx->cmd_line.escape.size))
    {
        ...
        case 'C':
            /* カーソル右移動 */
            cli_cursor_move_right(ctx);
            break;

        case 'D':
            /* カーソル左移動 */
            cli_cursor_move_left(ctx);
            break;
